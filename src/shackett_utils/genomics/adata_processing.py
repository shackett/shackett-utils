...
def plot_feature_counts_histogram(
    ...
    if log_scale:
        ...
        counts, bin_edges, _ = ax.hist(
            feature_counts, bins=bins, alpha=0.7, color="steelblue", edgecolor="black"
        )
...
def filter_features_by_counts(
    ...
    n_features_before = (
        adata.n_vars if not inplace else filtered_adata.n_vars + np.sum(~keep_features)
    )
    ...
    if modify_source:
        if layer is None:
            ...
            if sp.issparse(adata.layers[layer]):
                adata.layers[layer].data = np.log2(
                    adata.layers[layer].data + pseudocount
                )
            else:
