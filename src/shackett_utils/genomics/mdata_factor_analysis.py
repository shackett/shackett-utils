...
@suppress_stdout
def _mofa(mdata: md.MuData, *args, **kwargs) -> None:
    """
    ...
    for n_factors in factor_range:
        ...
        try:
            ...
            model_data = create_minimal_mudata(
                mdata, include_layers=[use_layer] if use_layer else None
            )
...
def calculate_variance_metrics(
    ...
    for n_factors, paths in factor_results.items():
        try:
            ...
            print(
                f"Loaded metrics for {n_factors} factors. Total variance: {metrics[n_factors]['total_variance']:.2f}%"
            )
            ...
            metrics[n_factors] = {
                ...
                VARIANCE_METRICS_DEFS.MODALITY_VARIANCE: {
                    mod: 0.0 for mod in (mdata.mod.keys() if mdata else [])
                },
...
def _determine_optimal_factors(
    ...
    factors = sorted(
        [
            ...
            if metrics[k][VARIANCE_METRICS_DEFS.TOTAL_VARIANCE] is not None
        ]
    )
    ...
    if criterion == K_SELECTION_CRITERIA.ELBOW:
        ...
        raise ValueError(
            f"Unknown criterion: {criterion}. Use {', '.join(AUTO_K_SELECTION_CRITERIA)}."
        )
...
def _visualize_variance_explained(
    ...
    if not factors:
        ...
        plt.text(
            0.5,
            0.5,
            "No valid variance metrics available",
    ...
    colors = {
        K_SELECTION_CRITERIA.ELBOW: "red",
        K_SELECTION_CRITERIA.THRESHOLD: "green",
        K_SELECTION_CRITERIA.BALANCED: "purple",
    }
    ...
    if user_factors and user_factors in factors:
        ...
        plt.scatter(
            [user_factors], [variance[idx]], marker="*", s=200, color="orange", zorder=5
        )
...
def plot_mofa_factor_histograms(
    ...
    for i, factor_idx in enumerate(factors):
        if i < len(axes):
            ...
            # Plot histogram
            sns.histplot(factor_values, bins=n_bins, kde=kde, color=color, ax=ax)
...
def visualize_factor_scan_results(
    ...
    optimal_factors = {
        K_SELECTION_CRITERIA.ELBOW: _determine_optimal_factors(
            metrics, criterion=K_SELECTION_CRITERIA.ELBOW
        ),
        K_SELECTION_CRITERIA.THRESHOLD: _determine_optimal_factors(
            metrics, criterion=K_SELECTION_CRITERIA.THRESHOLD, threshold=0.01
        ),
        K_SELECTION_CRITERIA.BALANCED: _determine_optimal_factors(
            metrics, criterion=K_SELECTION_CRITERIA.BALANCED
        ),
    }
    ...
    figures["total_variance"] = _visualize_variance_explained(
        metrics, optimal_factors, user_factors, figsize=figsize, save_path=save_path
    )
...
def _determine_optimal_elbow(
    factors: List[int], variance: List[float], diffs: np.ndarray
) -> int:
    ...
    elbow_idx = (
        np.argmax(np.abs(acceleration)) + 1
    )  # +1 because we're working with diffs

    return factors[elbow_idx]


def _determine_optimal_threshold(
    factors: List[int], diffs: np.ndarray, threshold: float
) -> int:
...
def _determine_optimal_balanced(factors: List[int], variance: List[float]) -> int:
    ...
    norm_variance = (np.array(variance) - min(variance)) / (
...
def _visualize_marginal_improvement(
    ...
    # Extract factors and variance values
    factors = sorted(
        [k for k in metrics.keys() if metrics[k]["total_variance"] is not None]
    )
    ...
    if len(factors) <= 1:
        ...
        plt.text(
            0.5,
            0.5,
            "Need at least two factor values to calculate marginal improvement",
            horizontalalignment="center",
            verticalalignment="center",
        )
        return fig
    ...
    if (
        user_factors
        and user_factors in factors
        and user_factors not in highlighted_factors
    ):
        idx = factors.index(user_factors)
        plt.bar(
            [user_factors],
...
def _visualize_modality_variance(
    ...
    for n_factors in factors:
        if metrics[n_factors][VARIANCE_METRICS_DEFS.MODALITY_VARIANCE]:
            ...
            for mod, var in mod_vars.items():
                if var is not None:
                    mod_data.append(
                        {"factors": n_factors, "modality": mod, "variance": var}
                    )
    ...
    if not mod_data:
        ...
        plt.text(
            0.5,
            0.5,
            "No modality-specific variance data available",
            horizontalalignment="center",
            verticalalignment="center",
        )
        return fig
    ...
    sns.lineplot(
        data=mod_df, x="factors", y="variance", hue="modality", marker="o", markersize=8
    )
...
def _factor_name_to_idx(name: str) -> int:
    ...
    try:
        ...
        raise ValueError(
            f"Invalid factor name '{name}'. Expected format: 'Factor_N' where N is a positive integer."
        ) from e
