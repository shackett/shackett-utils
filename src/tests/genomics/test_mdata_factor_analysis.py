...
def test_mofa_factor_analysis(simple_mdata, tmp_path):
    ...
    factor_results = mfa.run_mofa_factor_scan(
        simple_mdata,
        factor_range=[3, 5],  # Test with 3 and 5 factors to ensure we can capture all patterns
        use_layer="log2_centered",
        use_var=None,  # Explicitly disable variable selection
    ...
    for n_factors in [3, 5]:
        ...
        assert set(metrics[n_factors].keys()) == {"total_variance", "modality_variance", "raw_tss", "raw_ess"}
        ...
        for modality in ["rna", "atac"]:
            ...
            assert 20 <= metrics[n_factors]["modality_variance"][modality] <= 100, \
            ...
            # ESS should not exceed TSS
            assert metrics[n_factors]["raw_ess"][modality] <= metrics[n_factors]["raw_tss"][modality]
        ...
        assert min_var <= metrics[n_factors]["total_variance"] <= max_var, \
        ...
        if n_factors == 5:
            assert metrics[n_factors]["total_variance"] >= metrics[3]["total_variance"], \
                "More factors should explain more or equal variance"
            
            # Check the increase is not unrealistically large
            variance_increase = metrics[n_factors]["total_variance"] - metrics[3]["total_variance"]
            assert variance_increase <= 30, \
...
def test_regress_factors_with_formula(simple_mdata):
    ...
    assert abs(factor3_group["estimate"].iloc[0]) > 0.3  # Should have moderate effect size
    assert abs(factor3_cont["estimate"].iloc[0]) > 0.3  # Should have moderate effect size
