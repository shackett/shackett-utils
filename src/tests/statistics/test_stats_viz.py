...
def test_plot_term_pvalue_histogram_options(sample_data):
    ...
    # Test without stats
    fig = stats_viz.plot_term_pvalue_histogram(term_data, "term1", include_stats=False)
    assert isinstance(fig, plt.Figure)
    plt.close(fig)

    # Test without KS test
    fig = stats_viz.plot_term_pvalue_histogram(term_data, "term1", show_ks_test=False)
    assert isinstance(fig, plt.Figure)
    plt.close(fig)

    # Test with custom figsize
    fig = stats_viz.plot_term_pvalue_histogram(term_data, "term1", figsize=(8, 3))
    assert isinstance(fig, plt.Figure)
...
def test_plot_pvalue_histograms_basic(sample_data):
    ...
    figures = stats_viz.plot_pvalue_histograms(sample_data, terms=["term1"])
    assert isinstance(figures, dict)
...
def test_plot_pvalue_histograms_partitioned(sample_data):
    ...
    figures = stats_viz.plot_pvalue_histograms(
        sample_data, partition_column="data_modality"
    )
    assert isinstance(figures, dict)
    assert set(figures.keys()) == {"term1", "term2"}
    assert all(isinstance(term_dict, dict) for term_dict in figures.values())
    assert all(
        set(term_dict.keys()) == {"RNA", "ATAC"} for term_dict in figures.values()
    )
    ...
    figures = stats_viz.plot_pvalue_histograms(
        sample_data, partition_column="data_modality", partition_values=["RNA"]
    )
    assert isinstance(figures, dict)
    assert all(set(term_dict.keys()) == {"RNA"} for term_dict in figures.values())
...
def test_plot_pvalue_histograms_validation(sample_data):
    ...
    with pytest.raises(ValueError, match="Terms not found in data"):
        stats_viz.plot_pvalue_histograms(sample_data, terms=["nonexistent_term"])
    ...
    with pytest.raises(KeyError):
        stats_viz.plot_pvalue_histograms(
            sample_data, partition_column="nonexistent_column"
        )
...
def test_plot_pvalue_histograms_empty_combinations(sample_data):
    ...
    figures = stats_viz.plot_pvalue_histograms(
        filtered_data, partition_column="data_modality"
    )
