...
def test_transform_filtering():
    ...
    for name, result in filtered.items():
        ...
        if not np.isnan(result["p"]):
            ...
            assert _is_valid_transform(result["transformed"]), f"{name} should have valid values"
...
def test_edge_cases():
    ...
    s = pd.Series([1])
    with pytest.raises(ValueError, match="Data must have at least 2 unique values for transformation"):
