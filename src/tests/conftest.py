...
from shackett_utils.genomics.constants import MOFA_DEFS
...
def simple_mdata():
    ...
    pattern1 = (
        3.0 * np.sin(np.linspace(0, 4 * np.pi, n_obs))[:, np.newaxis]
    )  # Amplify RNA signal
    pattern2 = (
        3.0 * np.cos(np.linspace(0, 4 * np.pi, n_obs))[:, np.newaxis]
    )  # Amplify RNA signal
    ...
    pattern3 = (
        5.0 * np.sin(np.linspace(0, 2 * np.pi, n_obs))[:, np.newaxis]
    )  # Strong ATAC signal
    pattern4 = (
        5.0 * np.cos(np.linspace(0, 2 * np.pi, n_obs))[:, np.newaxis]
    )  # Strong ATAC signal
    ...
    X2 += np.random.normal(
        0, 0.5, (n_obs, n_vars2)
    )  # More noise but still dominated by signal
    ...
    # Create modalities
    rna = ad.AnnData(
        X=X1, obs=obs, var=pd.DataFrame(index=[f"gene_{i}" for i in range(n_vars1)])
    )

    atac = ad.AnnData(
        X=X2, obs=obs, var=pd.DataFrame(index=[f"peak_{i}" for i in range(n_vars2)])
    )
    ...
    X_mofa[:, 2] = (
        (obs["group"] == "A").astype(float)
        + 0.5 * obs["continuous_var"]
    ...
    # Add to mdata.obsm
    mdata.obsm[MOFA_DEFS.X_MOFA] = X_mofa
